...
DLManagedTensor* cupoch::utility::ToDLPack(const thrust::device_vector<Eigen::Vector3f>& src) {
...
thrust::device_vector<Eigen::Vector3f> cupoch::utility::FromDLPack(const DLManagedTensor* src) {
    return thrust::device_vector<Eigen::Vector3f>();
}
