...
struct convert_from_rgbdimage_functor{
    convert_from_rgbdimage_functor(const uint8_t* depth,
                                   const uint8_t* color,
                                   int width,
                                   const Eigen::Matrix4f& camera_pose,
                                   const thrust::pair<float, float>& principal_point,
                                   ...
                                   : depth_(depth), color_(color), width_(width),
    ...
    thrust::tuple<Eigen::Vector3f, Eigen::Vector3f> operator() (size_t idx) const {
        ...
        if (*p > 0) {
            ...
            return thrust::make_tuple(Eigen::Vector3f(x, y, z),
...
std::shared_ptr<PointCloud> CreatePointCloudFromRGBDImageT(
    ...
    convert_from_rgbdimage_functor<TC, NC> func(thrust::raw_pointer_cast(image.depth_.data_.data()),
                                                thrust::raw_pointer_cast(image.color_.data_.data()),
                                                image.depth_.width_,
                                                camera_pose,
                                                principal_point, focal_length, scale,
