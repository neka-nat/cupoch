...
public:
    ...
    bool HasWeights() const {
        return edge_weights_.size() > 0 && this->lines_.size() == edge_weights_.size();
    };

    bool HasNodeColors() const {
        return node_colors_.size() > 0 && this->points_.size() == node_colors_.size();
    };

    bool IsConstructed() const {
        return edge_index_offsets_.size() == this->points_.size() + 1 && this->lines_.size() == edge_weights_.size();
    }

    Graph<Dim> &Clear() override;

    Graph<Dim> &ConstructGraph(bool set_edge_weights_from_distance = true);
    Graph<Dim> &ConnectToNearestNeighbors(float max_edge_distance, int max_num_edges = 30);
    Graph<Dim> &AddNodeAndConnect(const Eigen::Matrix<float, Dim, 1>& point, float max_edge_distance = 0.0f, bool lazy_add = false);

    Graph<Dim> &AddEdge(const Eigen::Vector2i &edge, float weight = 1.0, bool lazy_add = false);
    ...
    Graph<Dim> &RemoveEdges(const utility::device_vector<Eigen::Vector2i> &edges);
    Graph<Dim> &RemoveEdges(const thrust::host_vector<Eigen::Vector2i> &edges);

    Graph<Dim> &PaintEdgeColor(const Eigen::Vector2i &edge, const Eigen::Vector3f &color);
    Graph<Dim> &PaintEdgesColor(const utility::device_vector<Eigen::Vector2i> &edges, const Eigen::Vector3f &color);
    Graph<Dim> &PaintEdgesColor(const thrust::host_vector<Eigen::Vector2i> &edges, const Eigen::Vector3f &color);

    Graph<Dim> &PaintNodeColor(int node, const Eigen::Vector3f &color);
    Graph<Dim> &PaintNodesColor(const utility::device_vector<int> &nodes, const Eigen::Vector3f &color);
    Graph<Dim> &PaintNodesColor(const thrust::host_vector<int> &nodes, const Eigen::Vector3f &color);
    ...
    static std::shared_ptr<Graph<Dim>> CreateFromTriangleMesh(const TriangleMesh &input);
    static std::shared_ptr<Graph<Dim>> CreateFromAxisAlignedBoundingBox(const geometry::AxisAlignedBoundingBox& bbox,
                                                                        const Eigen::Vector3i& resolutions);
    static std::shared_ptr<Graph<Dim>> CreateFromAxisAlignedBoundingBox(const Eigen::Matrix<float, Dim, 1>& min_bound,
