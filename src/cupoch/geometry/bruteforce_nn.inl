...
template<int Dim>
__global__
void ComputeDistancesKernel(const Eigen::Matrix<float, Dim, 1>* ref, const Eigen::Matrix<float, Dim, 1>* query,
                            float* distance_mat, int ref_size, int query_size) {
...
__global__
void FindNNKernel(const float* distance_mat, int* indices, float* distances,
                  int ref_size, int query_size) {
...
template <int Dim>
void BruteForceNN(const utility::device_vector<Eigen::Matrix<float, Dim, 1>>& ref,
    ...
    ComputeDistancesKernel<<<blocks1, threads1>>>(thrust::raw_pointer_cast(ref.data()),
    ...
    FindNNKernel<<<blocks2, threads2>>>(thrust::raw_pointer_cast(distance_mat.data()),
