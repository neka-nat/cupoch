...
template <int Dim>
__host__ __device__
float Scale(int index, bool no_blur) {
    return (Dim + 1) * sqrtf(((no_blur) ? (1.0f / 6.0f) : (2.0f / 3.0f)) / ((index + 1) * (index + 2)));
}
...
void CreateLatticeGrid(float *feature,
    ...
    for (int i = 0; i <= Dim; ++i) {
        ...
        greedy[i] = static_cast<short>((up - elevated[i] < elevated[i] - down) ? up : down);
        sum += greedy[i];
    }
    sum /= Dim + 1;
    ...
    if (no_blur) {
        ...
        for (int i = 0; i < Dim; ++i) {
            ...
            for (int j = i + 1; j <= Dim; ++j) {
                ++rank[(elevated[i] - greedy[i] < elevated[j] - greedy[j]) ? i : j];
            }
        }
    } else {
        #pragma unroll
