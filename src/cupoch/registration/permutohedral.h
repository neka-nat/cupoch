...
struct LatticeInfo{
    __host__ __device__ LatticeInfo(float weight = 0,
    ...
    : weight_(other.weight_), vertex_(other.vertex_), vTv_(other.vTv_), normal_(other.normal_) {};

    __host__ __device__
    LatticeInfo& operator+=(const LatticeInfo& other) {
    ...
    __host__ __device__
    LatticeInfo& operator*=(float other) {
        weight_ *= other;
        vertex_ *= other;
        vTv_ *= other;
        normal_ *= other;
...
__host__ __device__
inline LatticeInfo operator+(const LatticeInfo& rhs, const LatticeInfo& lhs) {
    LatticeInfo ans = rhs;
    ans += lhs;
    return ans;
}
...
__host__ __device__
inline LatticeInfo operator*(float rhs, const LatticeInfo& lhs) {
    LatticeInfo ans = lhs;
    ans *= rhs;
    return ans;
}
...
public:
    ...
    struct PermutohedralHasher {
        __host__ __device__
        size_t operator()(const LatticeCoordKey<Dim>& lattice) const {
            return lattice.hash();
        }
    };
    typedef stdgpu::unordered_map<LatticeCoordKey<Dim>, LatticeInfo, PermutohedralHasher> MapType;

    Permutohedral(float sigma) : sigma_(Eigen::Vector3f::Constant(sigma)) {};
    ~Permutohedral();

    void BuildLatticeIndexNoBlur(const utility::device_vector<Eigen::Matrix<float, Dim, 1>>& obs_feature,
                                 const utility::device_vector<Eigen::Vector3f>& obs_vertex);

    void ComputeTarget(const utility::device_vector<Eigen::Matrix<float, Dim, 1>>& model_feature,
