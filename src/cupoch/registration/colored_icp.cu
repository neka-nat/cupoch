...
struct compute_color_gradient_functor {
    compute_color_gradient_functor(const Eigen::Vector3f* points, const Eigen::Vector3f* normals,
                                   const Eigen::Vector3f* colors,
                                   const int* indices, const float* distances2, int knn)
                                   : points_(points), normals_(normals), colors_(colors),
                                     indices_(indices), distances2_(distances2), knn_(knn) {};
    ...
    Eigen::Vector3f operator()(size_t idx) const {
        ...
        float it = (colors_[idx](0) + colors_[idx](1) +
                    colors_[idx](2)) / 3.0;
        ...
        for (size_t i = 1; i < knn_; ++i) {
            ...
            float it_adj = (colors_[P_adj_idx](0) +
                            colors_[P_adj_idx](1) +
                            colors_[P_adj_idx](2)) /
                           3.0;
...
struct compute_jacobian_and_residual_functor : public utility::multiple_jacobians_residuals_functor<Eigen::Vector6f, 2> {
    compute_jacobian_and_residual_functor(const Eigen::Vector3f* source_points, const Eigen::Vector3f* source_colors,
                                          const Eigen::Vector3f* target_points, const Eigen::Vector3f* target_normals,
                                          const Eigen::Vector3f* target_colors, const Eigen::Vector3f* target_color_gradient,
                                          const Eigen::Vector2i* corres,
                                          float sqrt_lambda_geometric, float sqrt_lambda_photometric)
                                          : source_points_(source_points), source_colors_(source_colors),
                                            target_points_(target_points), target_normals_(target_normals),
                                            target_colors_(target_colors), target_color_gradient_(target_color_gradient),
                                            corres_(corres),
    ...
    void operator() (int i, Eigen::Vector6f J_r[2], float r[2]) const {
        ...
        J_r[1].block<3, 1>(0, 0) =
                sqrt_lambda_photometric_ * vs.cross(ditM);
...
Eigen::Matrix4f TransformationEstimationForColoredICP::ComputeTransformation(
    ...
    compute_jacobian_and_residual_functor func(thrust::raw_pointer_cast(source.points_.data()),
    ...
    thrust::tie(JTJ, JTr, r2) =
            utility::ComputeJTJandJTr<Eigen::Matrix6f, Eigen::Vector6f, 2, compute_jacobian_and_residual_functor>(
...
struct diff_square_colored_functor {
    diff_square_colored_functor(const Eigen::Vector3f* source_points, const Eigen::Vector3f* source_colors,
                                const Eigen::Vector3f* target_points, const Eigen::Vector3f* target_normals,
                                const Eigen::Vector3f* target_colors, const Eigen::Vector3f* target_color_gradient,
                                const Eigen::Vector2i* corres,
                                float sqrt_lambda_geometric, float sqrt_lambda_photometric)
...
float TransformationEstimationForColoredICP::ComputeRMSE(
    ...
    diff_square_colored_functor func(thrust::raw_pointer_cast(source.points_.data()),
                                     thrust::raw_pointer_cast(source.colors_.data()),
                                     thrust::raw_pointer_cast(target.points_.data()),
                                     thrust::raw_pointer_cast(target.normals_.data()),
                                     thrust::raw_pointer_cast(target.colors_.data()),
                                     ...
                                     thrust::raw_pointer_cast(corres.data()),
                                     sqrt_lambda_geometric, sqrt_lambda_photometric);
    const auto err = thrust::transform_reduce(thrust::make_counting_iterator<size_t>(0), thrust::make_counting_iterator(corres.size()),
