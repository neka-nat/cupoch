...
struct weighted_residual_functor {
    __device__ float operator() (const thrust::tuple<Eigen::Vector3f, Eigen::Vector3f, float>& x) {
        const Eigen::Vector3f r = thrust::get<2>(x) * (thrust::get<0>(x) - thrust::get<1>(x));
...
FilterRegResult GetRegistrationResult(
    ...
    result.likelihood_ =
            thrust::transform_reduce(make_tuple_begin(model, target, weights), make_tuple_end(model, target, weights),
                                     weighted_residual_functor(), 0.0f,
                                     thrust::plus<float>());
    return result;
}
...
FilterRegResult RegistrationFilterReg(
    ...
    utility::device_vector<Eigen::Vector3f> target_pt(source.points_.size(), Eigen::Vector3f::Zero());
    ...
    for(int i = 0; i < option.max_iteration_; ++i) {
        ...
        Eigen::Matrix4f update = registration::KabschWeighted(model.points_, target_pt, weight);
        transform = update * transform;
        model.Transform(update);
        const auto sigma = pmh.ComputeSigma(model.points_, target_pt, weight, m2);
        ...
        result = GetRegistrationResult(model.points_, target_pt, weight, transform);
