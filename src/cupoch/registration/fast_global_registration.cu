...
struct compute_tuple_constraint_functor {
    compute_tuple_constraint_functor(size_t ncorr,
    ...
    __device__ void operator() (size_t idx) {
        ...
        bool cond = (li0 * scale_ < lj0) && (lj0 < li0 / scale_) && (li1 * scale_ < lj1) &&
            (lj1 < li1 / scale_) && (li2 * scale_ < lj2) && (lj2 < li2 / scale_);
        thrust::tuple<int, int> invalid_idx = thrust::make_tuple(-1, -1);
        corres_tuple_[3 * idx] = (cond) ? thrust::make_tuple(idi0, idj0) : invalid_idx;
        corres_tuple_[3 * idx + 1] = (cond) ? thrust::make_tuple(idi1, idj1) : invalid_idx;
        corres_tuple_[3 * idx + 2] = (cond) ? thrust::make_tuple(idi2, idj2) : invalid_idx;
    }
};
...
utility::device_vector<thrust::tuple<int, int>> AdvancedMatching(
    ...
    geometry::BruteForceNN(features_vec[fi].data_, features_vec[fj].data_, corresK, dis);
    thrust::copy(make_tuple_iterator(corresK.begin(), thrust::make_counting_iterator<int>(0)),
                 make_tuple_iterator(corresK.end(), thrust::make_counting_iterator<int>(corresK.size())),
                 corres.begin());
    geometry::BruteForceNN(features_vec[fj].data_, features_vec[fi].data_, corresK, dis);
    thrust::copy(make_tuple_iterator(thrust::make_counting_iterator<int>(0), corresK.begin()),
                 make_tuple_iterator(thrust::make_counting_iterator<int>(corresK.size()), corresK.end()),
                 corres.begin() + nPtj);
    ...
    auto end2 = thrust::remove_if(corres_cross.begin(), end1.first, counts.begin(),
    ...
    utility::device_vector<thrust::tuple<int, int>> corres_tuple(3 * number_of_trial);
    compute_tuple_constraint_functor func(ncorr,
                                          thrust::raw_pointer_cast(corres_cross.data()),
    ...
    auto end3 = thrust::remove_if(corres_tuple.begin(), corres_tuple.end(),
                                  [] __device__ (const thrust::tuple<int, int>& corr) { return thrust::get<0>(corr) < 0; } );
    size_t n_res = thrust::distance(corres_tuple.begin(), end3);
    corres_tuple.resize(std::min((int)n_res, option.maximum_tuple_count_));
    utility::LogDebug("{:d} tuples ({:d} trial, {:d} actual).", corres_tuple.size(),
                      number_of_trial, n_res);

    if (swapped) {
        thrust::for_each(corres_tuple.begin(), corres_tuple.end(),
                         [] __device__ (thrust::tuple<int, int>& corr) {
                            thrust::swap(thrust::get<0>(corr), thrust::get<1>(corr));
                         });
    }
...
std::tuple<std::vector<Eigen::Vector3f>, float, float> NormalizePointCloud(
    ...
    for (int i = 0; i < num; ++i) {
        Eigen::Vector3f mean = thrust::reduce(point_cloud_vec[i].points_.begin(),
        ...
        thrust::for_each(point_cloud_vec[i].points_.begin(), point_cloud_vec[i].points_.end(),
                         [mean] __device__ (Eigen::Vector3f& pt) {
                              pt -= mean;
                         });

        scale = thrust::transform_reduce(point_cloud_vec[i].points_.begin(),
                                         point_cloud_vec[i].points_.end(),
...
Eigen::Matrix4f GetInvTransformationOriginalScale(
    ...
    transtemp.block<3, 1>(0, 3) =
            -R.transpose() * (-R * pcd_mean_vec[1] + t * scale_global + pcd_mean_vec[0]);
    transtemp(3, 3) = 1;
    return transtemp;
}
