...
struct create_from_primitive_functor {
    ...
    const int num_h_;
    const int num_d_;
    __device__ virtual bool intersect(const Eigen::Vector3f& box_center) const = 0;
    __device__ thrust::tuple<Eigen::Vector3i, geometry::Voxel> operator()(
        ...
        const Eigen::Vector3f box_center = Eigen::Vector3f(widx + 0.5, hidx + 0.5, didx + 0.5) * voxel_size_;
...
struct create_from_box_functor : create_from_primitive_functor<Box> {
    using create_from_primitive_functor<Box>::create_from_primitive_functor<Box>;
    __device__ bool intersect(const Eigen::Vector3f& box_center) const {
        return geometry::intersection_test::BoxBox(0.5 * primitive_.lengths_, primitive_.transform_.block<3, 3>(0, 0),
...
struct create_from_sphere_functor : create_from_primitive_functor<Sphere> {
    using create_from_primitive_functor<Sphere>::create_from_primitive_functor<Sphere>;
    __device__ bool intersect(const Eigen::Vector3f& box_center) const {
        return geometry::intersection_test::SphereAABB(Eigen::Vector3f::Zero(), primitive_.radius_,
...
struct create_from_swept_primitive_functor {
    ...
    const int sampling_;
    __device__ virtual bool intersect(const Eigen::Matrix4f& primitive_trans,
    ...
    __device__ thrust::tuple<Eigen::Vector3i, geometry::Voxel> operator()(
        ...
        Eigen::Quaternionf tq = qdst.slerp(1.0 - ratio, Eigen::Quaternionf::Identity());
        Eigen::Matrix4f dtf = dst_;
        dtf.block<3, 1>(0, 3) *= ratio;
        dtf.block<3, 3>(0, 0) = tq.toRotationMatrix();
        Eigen::Vector3f box_center = (dtf * Eigen::Vector4f(voxel_size_ * (widx + 0.5),
                                                            voxel_size_ * (hidx + 0.5),
                                                            voxel_size_ * (didx + 0.5), 1.0)).head<3>();
...
struct create_from_swept_box_functor : create_from_swept_primitive_functor<Box> {
    using create_from_swept_primitive_functor<Box>::create_from_swept_primitive_functor<Box>;
    __device__ bool intersect(const Eigen::Matrix4f& primitive_trans,
        ...
        return geometry::intersection_test::BoxBox(0.5 * primitive_.lengths_,
                                                   t.block<3, 3>(0, 0), t.block<3, 1>(0, 3),
                                                   box_half_size_, Eigen::Matrix3f::Identity(),
                                                   box_center);
    }
};

struct create_from_swept_sphere_functor : create_from_swept_primitive_functor<Sphere> {
    using create_from_swept_primitive_functor<Sphere>::create_from_swept_primitive_functor<Sphere>;
    __device__ bool intersect(const Eigen::Matrix4f& primitive_trans,
                              const Eigen::Vector3f& box_center) const {
...
void TransformAndResizeVoxel(geometry::VoxelGrid& voxelgrid, size_t n_total, Func fn) {
    thrust::transform(thrust::make_counting_iterator<size_t>(0),
                      ...
                      fn);
    auto remove_fn = [] __device__(const thrust::tuple<Eigen::Vector3i, geometry::Voxel> &x) -> bool {
        Eigen::Vector3i idxs = thrust::get<0>(x);
        return idxs == Eigen::Vector3i(geometry::INVALID_VOXEL_INDEX,
                                       geometry::INVALID_VOXEL_INDEX,
                                       geometry::INVALID_VOXEL_INDEX);
    };
    remove_if_vectors(remove_fn, voxelgrid.voxels_keys_, voxelgrid.voxels_values_);
}
...
std::shared_ptr<geometry::VoxelGrid> CreateVoxelGridFromPrimitive(
    ...
    const Eigen::Vector3f min_bound = bbx.min_bound_ - output->origin_ - voxel_size3 * 0.5;
    const Eigen::Vector3f max_bound = bbx.max_bound_ - output->origin_ + voxel_size3 * 0.5;
...
std::shared_ptr<geometry::VoxelGrid> CreateVoxelGridWithSweepingFromPrimitive(
        const T& primitive, float voxel_size, const Eigen::Matrix4f& dst, int sampling) {
    auto output = std::make_shared<geometry::VoxelGrid>();
    if (voxel_size <= 0.0) {
        utility::LogError("[CreateVoxelGrid] voxel_size <= 0.");
        return output;
...
std::shared_ptr<geometry::VoxelGrid> CreateVoxelGrid(const Primitive& primitive, float voxel_size) {
    switch (primitive.type_) {
        case Primitive::PrimitiveType::Box: {
            ...
            return CreateVoxelGridFromPrimitive<Box, create_from_box_functor>(box, voxel_size);
        }
...
std::shared_ptr<geometry::VoxelGrid> CreateVoxelGridWithSweeping(const Primitive& primitive,
    ...
    switch (primitive.type_) {
        case Primitive::PrimitiveType::Box: {
            const Box& box = (const Box&)primitive;
            return CreateVoxelGridWithSweepingFromPrimitive<Box, create_from_swept_box_functor>(box, voxel_size, dst, sampling);
        }
        case Primitive::PrimitiveType::Sphere: {
            const Sphere& sphere = (const Sphere&)primitive;
...
std::shared_ptr<geometry::TriangleMesh> CreateTriangleMesh(const Primitive& primitive) {
    switch (primitive.type_) {
        ...
        case Primitive::PrimitiveType::Capsule: {
            ...
            auto output = geometry::TriangleMesh::CreateCylinder(capsule.radius_, capsule.height_);
            output->Transform(primitive.transform_);
            return output;
        }
        default: {
