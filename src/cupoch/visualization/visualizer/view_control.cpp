...
const float ViewControl::FIELD_OF_VIEW_MAX = 90.0;
const float ViewControl::FIELD_OF_VIEW_MIN = 5.0;
const float ViewControl::FIELD_OF_VIEW_DEFAULT = 60.0;
const float ViewControl::FIELD_OF_VIEW_STEP = 5.0;

const float ViewControl::ZOOM_DEFAULT = 0.7;
const float ViewControl::ZOOM_MIN = 0.02;
const float ViewControl::ZOOM_MAX = 2.0;
const float ViewControl::ZOOM_STEP = 0.02;

const float ViewControl::ROTATION_RADIAN_PER_PIXEL = 0.003;
...
bool ViewControl::ConvertToPinholeCameraParameters(
    ...
    intrinsic.intrinsic_matrix_(0, 0) = intrinsic.intrinsic_matrix_(1, 1) =
            (float)window_height_ / tan_half_fov / 2.0;
    intrinsic.intrinsic_matrix_(0, 2) = (float)window_width_ / 2.0 - 0.5;
    intrinsic.intrinsic_matrix_(1, 2) = (float)window_height_ / 2.0 - 0.5;
    parameters.intrinsic_ = intrinsic;
    Eigen::Matrix4f extrinsic;
    extrinsic.setZero();
    Eigen::Vector3f front_dir = front_.normalized();
    Eigen::Vector3f up_dir = up_.normalized();
    Eigen::Vector3f right_dir = right_.normalized();
...
void ViewControl::Reset() {
    ...
    front_ = Eigen::Vector3f(0.0, 0.0, 1.0);
    SetProjectionParameters();
}
...
void ViewControl::ChangeFieldOfView(float step) {
    field_of_view_ =
...
void ViewControl::ChangeWindowSize(int width, int height) {
    ...
    aspect_ = (float)window_width_ / (float)window_height_;
    SetProjectionParameters();
}

void ViewControl::Scale(float scale) {
    zoom_ = std::max(std::min(zoom_ + scale * ZOOM_STEP, ZOOM_MAX), ZOOM_MIN);
    SetProjectionParameters();
}

void ViewControl::Rotate(float x,
                         float y,
                         float xo /* = 0.0*/,
                         float yo /* = 0.0*/) {
    // some black magic to do rotation
    float alpha = x * ROTATION_RADIAN_PER_PIXEL;
    float beta = y * ROTATION_RADIAN_PER_PIXEL;
...
void ViewControl::Translate(float x,
                            float y,
                            float xo /* = 0.0*/,
                            float yo /* = 0.0*/) {
    Eigen::Vector3f shift = right_ * (-x) / window_height_ * view_ratio_ * 2.0 +
