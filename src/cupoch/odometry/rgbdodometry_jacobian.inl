...
const float LAMBDA_HYBRID_DEPTH = 0.968;
...
void RGBDOdometryJacobianFromColorTerm::ComputeJacobianAndResidual(
    ...
    float dIdx = SOBEL_SCALE * (*geometry::PointerAt<float>(target_dx_color, width, u_t, v_t));
    float dIdy = SOBEL_SCALE * (*geometry::PointerAt<float>(target_dy_color, width, u_t, v_t));
    Eigen::Vector3f p3d_mat(*geometry::PointerAt<float>(source_xyz, width, 3, u_s, v_s, 0),
...
void RGBDOdometryJacobianFromHybridTerm::ComputeJacobianAndResidual(
    ...
    int u_t = corresps[row](2);
    int v_t = corresps[row](3);
    float diff_photo = (*geometry::PointerAt<float>(target_color, width, u_t, v_t) -
                        *geometry::PointerAt<float>(source_color, width, u_s, v_s));
    float dIdx = SOBEL_SCALE * (*geometry::PointerAt<float>(target_dx_color, width, u_t, v_t));
    float dIdy = SOBEL_SCALE * (*geometry::PointerAt<float>(target_dy_color, width, u_t, v_t));
    float dDdx = SOBEL_SCALE * (*geometry::PointerAt<float>(target_dx_depth, width, u_t, v_t));
    float dDdy = SOBEL_SCALE * (*geometry::PointerAt<float>(target_dy_depth, width, u_t, v_t));
    ...
    float diff_geo = *geometry::PointerAt<float>(target_depth, width, u_t, v_t) - p3d_trans(2);
    float invz = 1. / p3d_trans(2);
