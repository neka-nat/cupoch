...
def pointcloud_generator():
    yield "cube", cph.geometry.TriangleMesh.create_sphere().sample_points_uniformly(int(1e4)), 0.4
    ...
    mesh += cph.geometry.TriangleMesh.create_moebius(twists=1).translate((0, -d, 0))
    mesh += cph.geometry.TriangleMesh.create_moebius(twists=2).translate((d, -d, 0))
    yield "shapes", mesh.sample_points_uniformly(int(1e5)), 0.5

    yield "fragment", cph.io.read_point_cloud("../../testdata/fragment.ply"), 0.02
...
if __name__ == "__main__":
    ...
    for pcl_name, pcl, eps in pointcloud_generator():
        ...
        labels = np.array(pcl.cluster_dbscan(eps=eps, min_points=10, print_progress=True).cpu())
